...
pub async fn handle_prompt(
    ...
    let workdir = config.opencode_workdir.to_string_lossy().to_string();
    let prompt_text = text.to_string();
    let continue_session = !session.is_expired();

    let result = tokio::task::spawn_blocking(move || {
        executor::run_opencode(&opencode_bin, &workdir, &prompt_text, continue_session)
    })
    .await
